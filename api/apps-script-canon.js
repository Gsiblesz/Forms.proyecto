...
const CONFIG = {
  ...
  TARGETS: {
    'inventario-pt': {
      ...
      HEAD: ['entry_id','FECHA','TIPO','SEDE','EMPRESA','CODIGO','UND','PRODUCTOS','RESPONSABLE','CANTIDAD'],
      MODE: 'append'
    }
  }
};
...
function _asDateString(x){ if(!x) return ''; if(Object.prototype.toString.call(x)==='[object Date]'){ var y=x.getFullYear(),m=String(x.getMonth()+1).padStart(2,'0'),d=String(x.getDate()).padStart(2,'0'); return y+'-'+m+'-'+d; } var s=String(x).trim(); if(/^\d{4}-\d{2}-\d{2}$/.test(s)) return s; var d=new Date(s); if(!isNaN(d)){ var yy=d.getFullYear(),mm=String(d.getMonth()+1).padStart(2,'0'),dd=String(d.getDate()).padStart(2,'0'); return yy+'-'+mm+'-'+dd; } return s; }
