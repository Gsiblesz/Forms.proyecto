...
const DEFAULT_GS_URL = "https://script.google.com/macros/s/AKfycbxklVSWsrwPrSNiEYiqWZ4Cl2b4kIGZ2iwxGEIoh2aI5YaRJpm2-o8BpWleXkyTjhIkHA/exec"; // updated link
...
function main() {
  ...
  if (descEl && cfg.description) descEl.textContent = cfg.description;
    ...
    if (v) {
      ...
      // Siempre enviar formId para que el backend seleccione destino/formato
      const formKey = (cfg.id === 'registros') ? 'inventario-pt' : (cfg.id || '');
      if (formKey) link.searchParams.set("formId", formKey);
      // Para formularios distintos de 'registros', pasar sheet explícito; para 'registros' dejar que TARGETS decida
      if (cfg.id !== 'registros' && cfg.sheetTab) {
  ...
  form.addEventListener("submit", async (e) => {
    ...
    const metaProbe = {
      ...
      formId: (cfg?.id === 'registros') ? 'inventario-pt' : (cfg?.id || null),
